<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:45:"./application/admin/view2/plugin\setting.html";i:1490352994;s:44:"./application/admin/view2/public\layout.html";i:1490352994;}*/ ?>
<!doctype html>
<html>
<head>
...
<div class="page">
    ...
    <form class="form-horizontal" id="addEditSpecForm" method="post">
        <div class="ncap-form-default">
            <?php if(is_array($plugin['config']) || $plugin['config'] instanceof \think\Collection): $i = 0; $__LIST__ = $plugin['config'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$config): $mod = ($i % 2 );++$i;?>
            <dl class="row">
                <dt class="tit">
                ...
                <dd class="opt">
                    <?php if($config['type'] == 'select'): ?>
                        <select name="config[<?php echo $config['name']; ?>]">
                            <?php if(is_array($config['option']) || $config['option'] instanceof \think\Collection): $o = 0; $__LIST__ = $config['option'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$option): $mod = ($o % 2 );++$o;?>
